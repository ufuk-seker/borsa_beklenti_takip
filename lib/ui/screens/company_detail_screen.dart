...
class CompanyDetailScreen extends StatelessWidget {
          ...
          children: [
            ...
            Row(
              ...
              children: [
                ...
                Expanded(flex: 1, child: _buildMetricsSidebar(context, target)),
              ],
            ),
            const SizedBox(height: 32),
        ...
        child: Column(
          ...
          children: [
            ...
            Expanded(
              child: TabBarView(
                children: [
                  _buildLineChart(context, 'sales', AppTheme.accentGreen),
                  _buildLineChart(context, 'ebitda', AppTheme.accentBlue),
                ],
              ),
            ),
          ],
  ...
  Widget _buildLineChart(BuildContext context, String metric, Color color) {
    final provider = Provider.of<BorsaProvider>(context);
  ...
  Widget _buildMetricsSidebar(BuildContext context, dynamic target) {
    ...
    return Column(
      children: [
        _buildStatTile("Yıllık Satış Hedefi", target.sales?.toStringAsFixed(0) ?? "0", Icons.leaderboard, progress: salesProgress, progressColor: AppTheme.accentGreen),
        const SizedBox(height: 16),
        _buildStatTile("Hedef FAVÖK", target.ebitda?.toStringAsFixed(0) ?? "0", Icons.pie_chart, progress: ebitdaProgress, progressColor: AppTheme.accentBlue),
        const SizedBox(height: 16),
  ...
  Widget _buildStatTile(String label, String value, IconData icon, {double? progress, Color? progressColor}) {
    return Container(
  ...
  Widget _buildQuarterlyTable(BuildContext context, BorsaProvider provider) {
    ...
    final companyActuals = provider.actuals.where((a) => a.companyId == company.id && a.year == year).toList();
    final target = provider.targets.firstWhere(
      (t) => t.companyId == company.id && t.year == year,
      orElse: () => FinancialTarget(companyId: company.id, year: year),
    );
  ...
  TableRow _buildTableRow(String p1, String p2, String p3, String p4) {
    // This helper might be redundant now, but keeping signature for compatibility if needed elsewhere
    return TableRow(
      children: [
