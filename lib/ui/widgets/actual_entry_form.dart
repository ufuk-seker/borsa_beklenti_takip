...
class _ActualEntryFormState extends State<ActualEntryForm> {
  ...
  void initState() {
    ...
    _selectedQuarter = widget.existingActual?.quarter ?? widget.initialQuarter ?? 1;
    _loadUserName();
  }
      ...
      child: SingleChildScrollView(
        child: Column(
          ...
          children: [
            ...
            _buildField("Gerçekleşen Satış*", _salesController, errorText: _salesError),
            const SizedBox(height: 16),
            _buildField("Gerçekleşen FAVÖK*", _ebitdaController, errorText: _ebitdaError),
            const SizedBox(height: 16),
            _buildField("Yatırım Miktarı (Opsiyonel)", _investmentController),
            const SizedBox(height: 16),
            _buildField("Girişi Yapan*", _addedByController, keyboardType: TextInputType.text, errorText: _addedByError),
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                onPressed: () {
                  ...
                  setState(() {
                    _salesError = _salesController.text.trim().isEmpty ? "Gerekli" : null;
                    _ebitdaError = _ebitdaController.text.trim().isEmpty ? "Gerekli" : null;
                    _addedByError = _addedByController.text.trim().isEmpty ? "Gerekli" : null;
                  });

                  if (_salesError != null || _ebitdaError != null || _addedByError != null) {
                    return;
                  }
                  
                  if (_addedByController.text.isNotEmpty) {
                    _saveUserName(_addedByController.text.trim());
                  }
  ...
  Widget _buildField(String label, TextEditingController controller, {TextInputType keyboardType = TextInputType.number, String? errorText}) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
